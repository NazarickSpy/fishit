...
local function SendWebhook(message)
    ...
    pcall(function()
        HttpService:PostAsync(Config.WebhookURL, jsonData)
    end)
end
        ...
        if Config.MultiCast then
            for i = 1, Config.CastAmount do
                task.spawn(function()
                    ...
                    if Config.WebhookEnabled and Config.WebhookURL ~= "" and Stats.FishCaught % Config.WebhookNotifyThreshold == 0 and Stats.FishCaught ~= Stats.LastWebhookNotify then
...
CreateToggle("Auto Buy Event", "AutoBuyEventEnabled", 1, AutoBuySection, function()
...
for i, event in ipairs(EventList) do
    CreateToggle(event, "", i, EventSelectionSection, function()
